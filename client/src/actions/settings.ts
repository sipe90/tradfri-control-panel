...
const loadCircadionSettingsSuccess = (settings: CircadianSettings | null) => ({
    payload: settings,
...
export const fetchCircadianSettings = (): AsyncThunkResult => async (dispatch) => {
    try {
        ...
        const res = await fetchGetJson<CircadianSettings>('/api/settings/circadian')
...
export const saveCircadianSettings = (settings: UpdateCircadianSettingsRequest): AsyncThunkResult =>
    async (dispatch) => {
        try {
